...
We don't have to define both GraphQL schema and REST serialization function from scratch. For GraphQL we can use `graphene-sqlalchemy` library, while in REST `marshmallow-sqlalchemy` library to build schema upon SQLAlchemy ORM models. It can give you a head start and fast iteration but has also huge drawback. You share the domain model in the public contract (with application clients, like frontend or other applications). So you must consider all pros and cons whether to do it.
